...
xmlhttp.open("GET","/conf/navi.xml",false);
xmlhttp.send();
...
document.write("<ul>");
var naviList = xmlDoc.getElementsByTagName("navi")[0].getElementsByTagName("name");
for (var i=0;i<naviList.length;i++){
	document.write("<li><a href='");
	document.write(xmlDoc.getElementsByTagName("navi")[0].getElementsByTagName("link")[i].childNodes[0].nodeValue);
	document.write("'>");
	...
	$("#navi").click(function () {
		if (ns == 0) {
			...
			$("#body").css({zIndex:100}).animate({left : 300}, 300, function () {
				$("#navi .list").css({display:"block"});
				$("#navi .text").css({display:"block"});
				var listC = 0;
				$("#navi .list ul li").each(function(){
			...
			$("#body").css({zIndex:100}).animate({left : 0}, 300, function () {
				ns = 0;
				$("#navi .text").css({display:"none"});
				$("#navi .list").css({display:"none"});
				$("#navi .list ul li").css({left : -300});
				$("#navi").css({width : 50,height : 50});
				$(this).css({zIndex:0});
			});
		}
	});
